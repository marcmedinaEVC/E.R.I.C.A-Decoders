...
def decoder_nevion_alarms_csv(control_ip):
    with open('D:/Nevion Data/Nevion Alarms/nevion_alarms' + decoder_name(control_ip) + '.csv', 'w') as f:
...
def decoder_analyze_nevions(control_ip):
    ini_path = 'D:/ERICA Data/Data/Decoders/Nevions TVG425 Alarms/'
    with open(ini_path + decoder_name(control_ip) + '_Alarm_Log.csv') as f:
    ...
    for log in ini_path:
        ...
        print('_' * 40)
        with open(
                ini_path + '/' + 'Nevion_Critical_Alarms__' + str(decoder_date_and_time()) + '_' + nevion_name + '.csv',
                'w') as critical_log:
...
def decoders_analyze_alarm_events(control_ip):
    ...
    # 'w') as file: file.write(data_raw) file.close() x = pd.read_csv('nevion_alarms.csv', delimiter=',') alarms = [] for
    # index, row in x.iterrows(): # print(row['On time'], row['Off time'], row['Sequence#'], row['Severity#'],
    # row['Severity'], row['Type'], row['Alarm ID'], #       row['Text'], row['Source'], row['Subid1'], row['Subid2'],
    # row['Subid3'], row['Details']) if row['Severity#'] == 6: print(
